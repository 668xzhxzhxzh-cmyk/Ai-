...
const toneClasses: Record<Tone, string> = {
  ...
  success: "border-lime-300/25 bg-lime-300/10 text-lime-100",
...
export function SectionCard({
  ...
  return (
    <section
      className={clsx(
        "rounded-[1.35rem] border bg-[linear-gradient(180deg,rgba(255,255,255,0.055),rgba(255,255,255,0.025))] p-4 shadow-[0_22px_80px_rgba(0,0,0,0.28)] sm:p-5",
        accent === "neutral" && "border-white/10",
        accent === "lime" && "border-lime-300/20 shadow-lime-950/10",
        accent === "sky" && "border-sky-300/20 shadow-sky-950/10",
  ...
  return (
    <button
      ...
      className={clsx(
        ...
        variant === "primary" && "bg-lime-300 text-black shadow-[0_10px_30px_rgba(190,242,100,0.18)] hover:bg-lime-200",
        variant === "secondary" && "border border-white/12 bg-white/[0.06] text-zinc-100 hover:border-white/20 hover:bg-white/[0.1]",
...
export function TextBlock({ children, className = "" }: { children?: string | null; className?: string }) {
  return (
    <div className={clsx("min-h-32 whitespace-pre-wrap rounded-[1.1rem] border border-white/10 bg-black/30 p-4 text-sm leading-7 text-zinc-300", className)}>
      {children || "暂无内容"}
    </div>
  );
}
...
export const inputClass =
  "focus-ring min-h-11 w-full rounded-2xl border border-white/10 bg-black/35 px-3.5 text-sm text-zinc-50 placeholder:text-zinc-600 transition-colors hover:border-white/20";
...
export function StatCard({
  ...
  return (
    <div className={clsx("rounded-[1.2rem] border p-4", toneClasses[tone])}>
...
export function ScoreRing({
  ...
  const color = tone === "lime" ? "#bef264" : tone === "sky" ? "#7dd3fc" : "#fcd34d";
  return (
    <div
      className={clsx(
        "grid place-items-center rounded-full border border-white/10 bg-black/35",
...
export function InsightCard({
  ...
  return (
    <div className={clsx("rounded-[1.15rem] border p-4", toneClasses[tone])}>
    ...
    <SectionCard accent="lime">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          <span className="grid h-10 w-10 place-items-center rounded-2xl bg-lime-300 text-black">
      ...
      <div className="grid gap-2">
        {items.map((item) => (
          <div key={`${item.name}-${item.meta}`} className="rounded-2xl border border-white/10 bg-black/25 p-3">
...
export function EmptyState({ title, description, action }: { title: string; description?: string; action?: ReactNode }) {
  return (
    <div className="rounded-[1.2rem] border border-dashed border-white/15 bg-white/[0.03] p-6 text-center">
      <p className="text-sm font-semibold text-zinc-100">{title}</p>
...
export function Notice({
  ...
  return (
    <div
      className={clsx(
        "rounded-2xl border p-3.5 text-sm leading-6",
        tone === "neutral" && toneClasses.neutral,
        tone === "success" && toneClasses.success,
        tone === "warning" && toneClasses.warning,
        tone === "danger" && toneClasses.danger,
